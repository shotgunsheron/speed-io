...
function drawCircle(centerX, centerY, radius, sides) {
    ...
    for (var i = 0; i < sides; i++) {
        theta = (i / sides) * 2 * Math.PI * Math.random() + 20;
        x = centerX + radius * Math.sin(theta + 3);
        y = centerY + radius * Math.cos(theta +6);
        graph.lineTo(x, y);
    }
...
function drawPlayers(order) {
    ...
    for(var z=0; z<order.length; z++)
    {
        ...
        var points = 30 + ~~(cellCurrent.mass/5*0.5);
        var increase = 7 * 2 / points;
...
function drawgrid() {
     ...
     //graph.globalAlpha = Math.random();
     graph.globalAlpha = 0.15;
     //graph.globalCompositeOperation = "source-in";
     graph.fillStyle = "000000";
     graph.beginPath();

    for (var x = global.xoffset - player.x; x < global.screenWidth; x += global.screenHeight / 4) {
        graph.moveTo(x, 0);
        graph.lineTo(x, global.screenHeight);
    }

    for (var y = global.yoffset - player.y ; y < global.screenHeight; y += global.screenHeight / 4) {
        graph.moveTo(0, y);
