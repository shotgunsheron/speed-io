...
function movePlayer(player) {
    ...
    {
        var target = {
            x: player.x - player.cells[i].x + player.target.x*Math.random(),
            y: player.y - player.cells[i].y + player.target.y*Math.random()
        };
        ...
        if (dist < (100 + player.cells[i].radius)) {
            deltaY *= dist / (100 + player.cells[i].radius);
            deltaX *= dist / (100 + player.cells[i].radius);
        }
        if (!isNaN(deltaY)) {
        ...
        if(player.cells.length > i) {
            var borderCalc = player.cells[i].radius / 20;
...
io.on('connection', function (socket) {
    ...
    socket.on('gotit', function (player) {
        ...
        if (util.findIndex(users, player.id) > -1) {
            ...
            //var radius = util.massToRadius(c.defaultPlayerMass);
            var radius = c.defaultPlayerMass;
        ...
        for(var i=0; i<currentPlayer.cells.length; i++)
        {
            //if(((currentPlayer.cells[i].mass >= c.defaultPlayerMass + c.fireFood) && c.fireFood > 0) || (currentPlayer.cells[i].mass >= 20 && c.fireFood === 0)){
                ...
                massFood.push({
                    ...
                    x: currentPlayer.cells[i].x,// - currentPlayer.cells[i].x*0.5 ,
                    y: currentPlayer.cells[i].y, //- currentPlayer.cells[i].x*0.5,
...
function tickPlayer(currentPlayer) {
    ...
    for(var z=0; z<currentPlayer.cells.length; z++) {
        ...
        currentCell.speed = currentCell.foodEaten*0.4;
